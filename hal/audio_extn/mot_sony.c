 ...
 * Modified for Motorola devices using Sony firmware
 *
...
#define LOG_TAG "audio_hw_motorola_cirrus_playback"
/*#define LOG_NDEBUG 0*/
...
#define PERSIST_CIRRUS_CAL_SPKL_CAL_R "/mnt/vendor/persist/factory/audio/spkl_cal_r"
#define PERSIST_CIRRUS_CAL_SPKL_CAL_STATUS "/mnt/vendor/persist/factory/audio/spkl_cal_status"
...
#define PERSIST_CIRRUS_CAL_SPKR_CAL_STATUS "/mnt/vendor/persist/factory/audio/spk_cal_status"
#define PERSIST_CIRRUS_CAL_SPKR_CAL_CHECKSUM "/mnt/vendor/persist/factory/audio/spk_checksum"
...
void log_mixer_controls(int card)
{
...
/* File-based calibration data handling instead of TA */
static int get_file_data(const char *filepath, void *buf, size_t size, bool reverse)
{
    FILE *fp = NULL;
...
void spkr_prot_init(void *adev, spkr_prot_init_config_t spkr_prot_init_config_val)
{
    ...
    ALOGI("%s: Using ambient values 0x%x 0x%x 0x%x 0x%x", __func__,
          cal_ambient[0], cal_ambient[1], cal_ambient[2], cal_ambient[3]);

#ifdef GET_SPEAKER_CALIBRATIONS_FROM_PERSIST
    /* Speaker LEFT */
    ret = get_file_data(PERSIST_CIRRUS_CAL_SPKL_CAL_R, &handle.spkl.cal_r, true);
    if (ret)
        return;

    ret = get_file_data(PERSIST_CIRRUS_CAL_SPKL_CAL_STATUS,
                        &handle.spkl.status, true);
    if (ret)
        return;

    ret = get_file_data(PERSIST_CIRRUS_CAL_SPKL_CAL_CHECKSUM,
                        &handle.spkl.checksum, true);
    if (ret)
        return;

    /* Speaker RIGHT */
    ret = get_file_data(PERSIST_CIRRUS_CAL_SPKR_CAL_R,
                        &handle.spkr.cal_r, true);
    if (ret)
        return;

    ret = get_file_data(PERSIST_CIRRUS_CAL_SPKR_CAL_STATUS,
                        &handle.spkr.status, true);
    if (ret)
        return;

    ret = get_file_data(PERSIST_CIRRUS_CAL_SPKR_CAL_CHECKSUM,
    ...
    if (ret == 0)
    {
        ...
        handle.spkr.cal_ok = true;
        ALOGI("%s: Successfully loaded calibration data", __func__);
    }
    else
    {
        handle.spkl.cal_ok = false;
        handle.spkr.cal_ok = false;
        ALOGI("%s: No valid calibration data found, calibration will be required", __func__);
    }
#endif
